...
import { Plug, ToggleLeft, ToggleRight, Zap, Database, Mail, MessageSquare, CreditCard, Archive, Layers, Users, Smartphone, Cloud, Key as KeyIcon, Check, Trash2 as TrashIcon } from 'lucide-react';

const ICON_MAP = { zap: Zap, database: Database, mail: Mail, 'message-square': MessageSquare, 'credit-card': CreditCard, archive: Archive, layers: Layers, users: Users, smartphone: Smartphone, cloud: Cloud };

// Friendly labels for the well-known connector secret keys.
const CRED_LABELS = {
  SLACK_WEBHOOK_URL: 'Slack Incoming Webhook URL',
  SLACK_BOT_TOKEN: 'Slack Bot Token',
  SENDGRID_API_KEY: 'SendGrid API Key',
  SENDGRID_FROM: 'SendGrid From Address',
  ...
  GOOGLE_ACCESS_TOKEN: 'Google Access Token (short-lived)',
  ...
  ZENDESK_BASE_URL: 'Zendesk Site URL',
  ...
  FRESHDESK_BASE_URL: 'Freshdesk Site URL',
  ...
  CALENDLY_TOKEN: 'Calendly Personal Token',
...
function CredentialManager() {
  const [data, setData] = useState({ data: [], env_configured: {}, known_keys: [] });
  const [loading, setLoading] = useState(true);
  const [drafts, setDrafts] = useState({});
  const { toast } = uT();

  async function load() {
    try { const r = await credsApi.list(); setData(r); } catch (e) { /* offline */ } finally { setLoading(false); }
  }
  useEffect(() => { load(); }, []);

  const stored = new Map((data.data || []).map(c => [c.name, c]));
  const envSet = data.env_configured || {};
  const keys = data.known_keys && data.known_keys.length ? data.known_keys : Object.keys(CRED_LABELS);
  ...
  async function save(name) {
    ...
    try {
      ...
      toast(`${CRED_LABELS[name] || name} saved`, 'success');
      setDrafts(d => ({ ...d, [name]: '' }));
      load();
  ...
  async function remove(name) {
    if (!confirm(`Delete stored ${CRED_LABELS[name] || name}?`)) return;
    try { await credsApi.delete(name); toast('Credential removed', 'info'); load(); }
  ...
  return (
    <div className="card" style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <KeyIcon size={15} />
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14 }}>Connector Credentials</span>
      </div>
      <div className="form-hint" style={{ marginBottom: 14 }}>
        Stored encrypted at rest. Values are never shown again after saving. A credential set here
        overrides the matching environment variable at runtime.
      </div>
      {loading ? <div className="skeleton" style={{ height: 200, borderRadius: 8 }} /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {keys.map(name => {
            const isStored = stored.has(name);
            const isEnv = !!envSet[name];
            return (
              <div key={name} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ flex: '0 0 200px', fontSize: 12 }}>
                  <div style={{ fontWeight: 600 }}>{CRED_LABELS[name] || name}</div>
                  <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{name}</div>
                </div>
                <input className="input" type="password" autoComplete="off"
                  placeholder={isStored ? '•••••••• stored (enter to replace)' : isEnv ? 'set via environment' : 'not configured'}
                  value={drafts[name] || ''}
                  onChange={e => setDrafts(d => ({ ...d, [name]: e.target.value }))}
                  style={{ flex: 1, fontSize: 12 }} />
                {isStored && <span className="badge badge-green" style={{ fontSize: 10 }}><Check size={10} />Stored</span>}
                {!isStored && isEnv && <span className="badge badge-muted" style={{ fontSize: 10 }}>env</span>}
                <button className="btn btn-secondary btn-sm" onClick={() => save(name)}>Save</button>
                {isStored && <button className="btn btn-ghost btn-icon btn-sm" onClick={() => remove(name)}><TrashIcon size={13} /></button>}
              </div>
            );
          })}
        </div>
...
export function Connectors() {
  ...
  useEffect(() => { connectorsApi.list().then(r => { setList(r.data || []); setLoading(false); }).catch(() => setLoading(false)); }, []);

  async function toggle(c) {
    try {
      await connectorsApi.toggle(c.id, !c.installed);
      setList(l => l.map(x => x.id === c.id ? { ...x, installed: !x.installed } : x));
      toast(`${c.name} ${!c.installed ? 'installed' : 'uninstalled'}`, 'success');
    } catch (e) { toast('Update failed', 'error', e.message); }
  }

  const byCategory = list.reduce((acc, c) => { (acc[c.category] = acc[c.category] || []).push(c); return acc; }, {});

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div className="page-header">
        <div><div className="page-title">Connector Registry</div><div className="page-subtitle">{list.filter(c => c.installed).length} installed · {list.length} available</div></div>
      </div>
      <div className="page-content">
        {loading ? <div className="skeleton" style={{ height: 400, borderRadius: 12 }} /> : (
          <>
            ...
            <div key={cat} style={{ marginBottom: 24 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>{cat}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
                {items.map(c => {
                  const Icon = ICON_MAP[c.icon] || Plug;
                  const ready = c.credentials_ready;
                  const needsCreds = c.credential_keys && c.credential_keys.length > 0;
                  return (
                    <div key={c.id} className="card" style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: c.installed ? 'var(--accent-dim)' : 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Icon size={16} color={c.installed ? 'var(--accent)' : 'var(--text-muted)'} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                          <span style={{ fontWeight: 600, fontSize: 13 }}>{c.name}</span>
                          {needsCreds && (ready
                            ? <span className="badge badge-green" style={{ fontSize: 9 }}><Check size={9} /> ready</span>
                            : <span className="badge badge-amber" style={{ fontSize: 9 }}>needs key</span>)}
                        </div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.description}</div>
                      </div>
                      <button className="btn btn-ghost btn-icon" onClick={() => toggle(c)} title={c.installed ? 'Uninstall' : 'Install'}>
                        {c.installed ? <ToggleRight size={22} color="var(--accent)" /> : <ToggleLeft size={22} color="var(--text-muted)" />}
                      </button>
                    </div>
                  );
                })}
